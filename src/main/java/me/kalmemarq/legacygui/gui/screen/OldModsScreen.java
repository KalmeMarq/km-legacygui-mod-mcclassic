...
public class OldModsScreen extends ExtraScreen {
    ...
    public OldModsScreen(Screen parent) {
    ...
    public void init() {
        if (this.mods.size() > 0) {
            ...
            this.selectedContainer = this.mods.get(0);
        }

        this.addWidget(new ButtonWidget( this.width / 2 + 5, this.height / 6 + 168 + 8, 155, 20, Language.translate("gui.done"), (button) -> {
            this.minecraft.openScreen(this.parent);
        }));

        this.addWidget(new ButtonWidget((this.width / 2 - 10) / 2 + 5 - 32 - 8, this.height / 6 + 168 - 14, 20, 20, "<", (button) -> {
    ...
    public void render(int mouseX, int mouseY) {
        ...
        for (int i = 0; i < maxDisplayed; i++) {
            ...
            if (this.selectedIndex == index) fill(baseX - 3, baseY - 1, this.width / 2 - 10 + 1, baseY + 37, this.selectedIndex == index ? 0xffffffff : 0xffbbbbbb);
            ...
            this.font.draw(name, baseX + 0 + 4, baseY + 2, 0xffffff);

            this.font.draw(lines[0], baseX + 0 + 4 + 2, baseY + 2 + 9, 0x999999);
            this.font.draw(lines[1], baseX + 0 + 4 + 2, baseY + 2 + 9 + 9, 0x999999);
        }
...
//            RenderHelper.bindTexture(this.minecraft, "classicmenu:mod_icon/" + this.selectedContainer.getMetadata().getId());
//            ModUtils.draw(baseX, baseY + 2, 32, 32, this.blitOffset);


            ModMetadata metadata = this.selectedContainer.getMetadata();

            int maxWidthTitle = this.width / 2 - 10 - (10 + 0 + 4) - 5;
            int maxWidth = this.width / 2 - 10 - (10 + 4) - 5;

            int y = 0;

            for (String line : TextUtil.splitToFit(metadata.getName(), this.font, maxWidthTitle)) {
                int xAdd = y > 34 ? 0 : 0;
            ...
            this.font.draw("Version: " + metadata.getVersion(), baseX + (y > 34 ? 0 : 0) + 4, baseY + 2 + y, 0x999999);

            if (y < 34) {
                y = 34;
            }

            for (String line : TextUtil.splitToFit(metadata.getDescription(), this.font, maxWidth)) {
